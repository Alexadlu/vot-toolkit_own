...
def check_center(bbox):
    ...
    # check data augmentation center
    cellx = 1. * w / S
    celly = 1. * h / S
    centerx = .5 * (bbox.x1 + bbox.x2)  # xmin, xmax
    centery = .5 * (bbox.y1 + bbox.y2)  # ymin, ymax
    cx = centerx / cellx
    cy = centery / celly
    if cx >= S or cy >= S or cx < 0 or cy < 0:
        return False
...
class example_generator:
    ...
    def make_training_example_BBShift_(self, bbParams, visualize_example=False):
        ...
        # config_coord for online, True -> False
        bbox_curr_shift = bbox_curr_gt.shift(self.img_curr_, bbParams.lamda_scale, bbParams.lamda_shift,
                                             bbParams.min_scale, bbParams.max_scale, False, bbox_curr_shift)
